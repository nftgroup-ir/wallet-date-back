...
def Chart(request):
    ...
    if request.GET['Type'] == 'NFT':
        ...
        if request.GET['TimeBase'] == 'day':
            ...
            for i in range(num_of_days):
                combine_array.append({'normal_date':send_information[i]['date'],"date" : time.mktime(datetime.strptime(str(send_information[i]['date']), "%Y-%m-%d").timetuple()), "send" : send_information[i]['value'], "receive" : receive_information[i]['value'],
